...
def redraw_tab2():
    # Clear the current contents of tab2
    for widget in tab2.winfo_children():
        widget.destroy()

    try:
        # Load the background image
        background_img2 = Image.open("bg2.png")
        background_img2 = ImageTk.PhotoImage(background_img2)

        # Create a label to display the background image
        background_label2 = Label(tab2, image=background_img2)
        background_label2.image = background_img2  # Keep a reference to prevent garbage collection
        background_label2.place(relx=0, rely=0, relwidth=1, relheight=1)  # Full-size placement
    except Exception as e:
        # Handle any exceptions, such as file not found or image loading errors
        print("Error:", e)
...
def run_create_avd_thread(avd_name, output_text):
    ...
    try:
        ...
        if return_code == 0:
            ...
            # Clear the output_text widget by deleting its contents
            output_text.delete(1.0, tk.END)
            redraw_tab2()
...
logo_image = PhotoImage(file="bg.png")
...
# Tab 3 ----------->
background_img3 = ImageTk.PhotoImage(Image.open("bg3.png")) 
